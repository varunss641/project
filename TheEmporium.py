...
def reg(msg=None):
    ...
    try:
        ...
        name = entAddName.get()
        password = entAddPas.get()
        cursor = con.cursor()
        sql = "insert into Pusers values ('%s','%s')"
        args = (name, password)

        if len(name) < 2:
            messagebox.showinfo(" username len cannot be less than two", msg)
        cursor.execute(sql % args)
        if name.isalpha():
            pass
        else:
            raise messagebox.showinfo("only string", msg)

        con.commit()
        msg = str(cursor.rowcount) + "registered"
        messagebox.showinfo("correct", msg)
        entAddName.delete(0, END)
        entAddPas.delete(0, END)
...
def forget():
    ...
    try:
        con = connect("system/123456")
        cursor = con.cursor()
        sql = "update Pusers set password='%s' where name='%s'"
        name = entUpdateName.get()
        password = entUpdatePa.get()
        args = (password, name)
...
def login():
    ...
    try:
        ...
        cursor = con.cursor()
        name = entlogName.get()
        password = entlogPas.get()
        sql = "select name, password from Pusers WHERE name='%s' AND password='%s' "
        cursor.execute(sql)
        row= cursor.fetchone()
        # if(row== None):
        ...
        if ((name == name) and (password == password)):
            ...
            page.deiconify()
            root.withdraw()
            entlogName.delete(0,END)
            entlogPas.delete(0,END)
        ...
        messagebox.showerror("Login Failure", e)
    finally:
...
filename = PhotoImage(file="G:\\TheEmporium-main\\pexels-shvets-production-7195232 (2).png")
background_label = Label(root, image=filename)
background_label.place(x=0, y=0, relwidth=1, relheight=1)
c.pack()

lbllogName = Label(root, text="Enter Name", font=("arial", 16, 'bold'), bg='coral1')
entlogName = Entry(root, bd=5, font=("arial", 16, 'bold'))
lbllogPas = Label(root, text="Enter Password", font=("arial", 16, 'bold'), bg='coral1')
entlogPas = Entry(root, bd=5, font=("arial", 16, 'bold'), show="*")
btnlogSave = Button(root, text="Login", font=("arial", 16, 'bold'), command=login)
# btnlogBack = Button(root, text="Back", font=("arial", 16, 'bold'), command=f2)
btnReg = Button(root, text="Register", font=("arial", 16, 'bold'), command=f2)
btnForgot = Button(root, text="Forgot Password", font=("arial", 16, 'bold'), command=f5)

lbllogName.pack(pady=10)
entlogName.pack(pady=10)
lbllogPas.pack(pady=10)
entlogPas.pack(pady=10)
btnlogSave.pack(pady=10)
# btnlogBack.pack(pady=10)
btnReg.pack(pady=10)
btnForgot.pack(pady=10)

# Register window

adst = Toplevel(root)
adst.title("Register")
adst.geometry("1360x1000")
adst.configure(background="red")
#c = Canvas(adst, bg="gray16", height=200, width=200)
#filename = PhotoImage(file="G:\\TheEmporium-main\\nathan-anderson-fcZIyU-nbFE-unsplash (1).png")
...
lblAddMks = Label(adst, text="Enter Password", font=("arial", 16, 'bold'), bg='coral1')
entAddPas = Entry(adst, bd=5, font=("arial", 16, 'bold'), show="*")
btnAddSave = Button(adst, text="Register", font=("arial", 16, 'bold'), command=reg)
# btnAddBack = Button(adst, text="Back", font=("arial", 16, 'bold'), command=f2)
btnAddlogin = Button(adst, text="Login", font=("arial", 16, 'bold'), command=f3)

lblAddName.pack(pady=10)
entAddName.pack(pady=10)
lblAddMks.pack(pady=10)
entAddPas.pack(pady=10)
btnAddSave.pack(pady=10)
# btnAddBack.pack(pady=10)
btnAddlogin.pack(pady=10)

# forgot
upst = Toplevel(root)
upst.title("Forgot password")
upst.geometry("1360x1000")
upst.configure(background="purple")
#c = Canvas(upst, bg="gray16", height=200, width=200)
#filename = PhotoImage(file="G:\\TheEmporium-main\\nathan-anderson-fcZIyU-nbFE-unsplash (1).png")
...
lblUpdatePa = Label(upst, text="Enter Password", width=30, font=("arial", 16, 'bold'), background='MediumOrchid1')
entUpdatePa = Entry(upst, bd=5, width=10, font=("arial", 16, 'bold'))
btnUpSave = Button(upst, text="Save", width=10, font=("arial", 16, 'bold'), command=forget)
btnUpBack = Button(upst, text="Back", width=10, font=("arial", 16, 'bold'), command=f6)

lblUpdateName.pack(pady=10)
entUpdateName.pack(pady=10)
lblUpdatePa.pack(pady=10)
entUpdatePa.pack(pady=10)
btnUpSave.pack(pady=10)
btnUpBack.pack(pady=10)

# e-commerce page
page = Toplevel(root)
page.title("Cartmax.in")
page.geometry("1360x1000")
page.withdraw()
Heading = LabelFrame(page, bd=2, relief="groove", bg="light yellow")
...
appliances_list = []
name = Label(Heading, text="The Emporium", font="arial 20 bold italic", bg="light pink", fg="blue").grid(row=0,
                                                                                                         column=1)
tagline = Label(Heading, text="Shopping made easier!", font="magneto 16 italic", fg="gold", bg="black").grid(row=0,
                                                                                                             column=2,
                                                                                                             padx=280)
Products_frame = LabelFrame(page, bd=2, relief="groove", text="Products", font="arial 16 bold", fg="dark blue")
...
Button_frame = LabelFrame(page, bd=2, relief="groove")
...
def save_invoice(text):
    ...
    c = 0
    file_name = "invoice" + "-" + str(c) + ".txt"
    if op:
        t = datetime.now()
        s = str(t.day) + str(t.month) + str(t.year) + str(t.hour) + str(t.minute) + str(t.second)
        f = open(file_name + s + ".txt", "w")
        f.write(text)
        messagebox.showinfo("Success", file_name + " created")
        c = c + 1
        f.close()
        messagebox.showinfo("Invoice Saving Status",
                            "Invoice is saved successfully as a text document with name " + s + ".txt")
...
def GroceryCall():
    HideAllFrames()
    Grocery_Label = Label(Products_frame, text="Grocery", font="times 15 bold", fg="gold", bg="black").grid(row=0,
                                                                                                            column=0,
                                                                                                            padx=20)
    ...
    name_grocery2 = Label(lf_grocery2, text="Aashirwaad Superior Atta", font="arial 9", justify="center").grid(row=1,
                                                                                                               column=0,
                                                                                                               padx=15)
    name_grocery3 = Label(lf_grocery3, text="Tata Iodized Salt", font="arial 9", justify="center").grid(row=1, column=0)
    name_grocery4 = Label(lf_grocery4, text="Safal Filtered Groundnut Oil", font="arial 9", justify="center").grid(
        row=1, column=0, padx=9)
    name_grocery5 = Label(lf_grocery5, text="24 Mantra Organic Toor Dal", font="arial 9", justify="center").grid(row=1,
                                                                                                                 column=0,
                                                                                                                 padx=9)
    name_grocery6 = Label(lf_grocery6, text="Dairy Milk Silk Fruit & Nut", font="arial 9", justify="center").grid(row=1,
                                                                                                                  column=0,
                                                                                                                  padx=15)
    name_grocery7 = Label(lf_grocery7, text="Yippee Noodles Magic Masala", font="arial 9", justify="center").grid(row=1,
                                                                                                                  column=0)
    name_grocery8 = Label(lf_grocery8, text="Kissan Mixed Fruit Jam", font="arial 9", justify="center").grid(row=1,
                                                                                                             column=0,
                                                                                                             padx=20)
    name_grocery9 = Label(lf_grocery9, text="Mother's Recipe Mango Pickle", font="arial 9", justify="center").grid(
        row=1, column=0, padx=2)
    name_grocery10 = Label(lf_grocery10, text="Parle's Cream & Onion Wafers", font="arial 9", justify="center").grid(
        row=1, column=0)
    ...
    def AddG1():
        ...
        if op:
            grocery_list.append(
                ["Corn Flakes", GroceryPrice(clicked_grocery1.get()), GroceryQty(clicked_grocery1.get()),
                 Spaces(40 - len("Corn Flakes"))])
            messagebox.showinfo("Product Status",
                                "Kellogg's Corn Flakes Original (" + clicked_grocery1.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Kellogg's Corn Flakes Original (" + clicked_grocery1.get() + ") is not added to the cart.")
    ...
    def AddG2():
        ...
        if op:
            grocery_list.append(
                ["Aashirwaad Atta", GroceryPrice(clicked_grocery2.get()), GroceryQty(clicked_grocery2.get()),
                 Spaces(40 - len("Aashirwaad Atta"))])
            messagebox.showinfo("Product Status",
                                "Aashirwaad Superior Atta (" + clicked_grocery2.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Aashirwaad Superior Atta (" + clicked_grocery2.get() + ") is not added to the cart.")
    ...
    def AddG3():
        ...
        if op:
            grocery_list.append(
                ["Iodized Salt", GroceryPrice(clicked_grocery3.get()), GroceryQty(clicked_grocery3.get()),
                 Spaces(40 - len("Iodized Salt"))])
            messagebox.showinfo("Product Status",
                                "Tata Iodized Salt (" + clicked_grocery3.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Tata Iodized Salt (" + clicked_grocery3.get() + ") is not added to the cart.")
    ...
    def AddG4():
        ...
        if op:
            grocery_list.append(
                ["Filtered Groundnut Oil", GroceryPrice(clicked_grocery4.get()), GroceryQty(clicked_grocery4.get()),
                 Spaces(40 - len("Filtered Groundnut Oil"))])
            messagebox.showinfo("Product Status",
                                "Safal Filtered Groundnut Oil (" + clicked_grocery4.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Safal Filtered Groundnut Oil (" + clicked_grocery4.get() + ") is not added to the cart.")
    ...
    def AddG5():
        ...
        if op:
            grocery_list.append(
                ["Organic Toor Dal", GroceryPrice(clicked_grocery5.get()), GroceryQty(clicked_grocery5.get()),
                 Spaces(40 - len("Organic Toor Dal"))])
            messagebox.showinfo("Product Status",
                                "24 Mantra Organic Toor Dal (" + clicked_grocery5.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "24 Mantra Organic Toor Dal (" + clicked_grocery5.get() + ") is not added to the cart.")
    ...
    def AddG6():
        ...
        if op:
            grocery_list.append(
                ["Dairy Milk Silk", GroceryPrice(clicked_grocery6.get()), GroceryQty(clicked_grocery6.get()),
                 Spaces(40 - len("Dairy Milk Silk"))])
            messagebox.showinfo("Product Status",
                                "Dairy Milk Silk Fruit & Nut (" + clicked_grocery6.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Dairy Milk Silk Fruit & Nut (" + clicked_grocery6.get() + ") is not added to the cart.")
    ...
    def AddG7():
        ...
        if op:
            grocery_list.append(
                ["Yippee Noodles", GroceryPrice(clicked_grocery7.get()), GroceryQty(clicked_grocery7.get()),
                 Spaces(40 - len("Yippee Noodles"))])
            messagebox.showinfo("Product Status",
                                "Yippee Noodles Magic Masala (" + clicked_grocery7.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Yippee Noodles Magic Masala (" + clicked_grocery7.get() + ") is not added to the cart.")
    ...
    def AddG8():
        ...
        if op:
            grocery_list.append(
                ["Mixed Fruit Jam", GroceryPrice(clicked_grocery8.get()), GroceryQty(clicked_grocery8.get()),
                 Spaces(40 - len("Mixed Fruit Jam"))])
            messagebox.showinfo("Product Status",
                                "Kissan Mixed Fruit Jam (" + clicked_grocery8.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Kissan Mixed Fruit Jam (" + clicked_grocery8.get() + ") is not added to the cart.")
    ...
    def AddG9():
        ...
        if op:
            grocery_list.append(
                ["Mango Pickle", GroceryPrice(clicked_grocery9.get()), GroceryQty(clicked_grocery9.get()),
                 Spaces(40 - len("Mango Pickle"))])
            messagebox.showinfo("Product Status",
                                "Mother's Recipe Mango Pickle (" + clicked_grocery9.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Mother's Recipe Mango Pickle (" + clicked_grocery9.get() + ") is not added to the cart.")
    ...
    def AddG10():
        ...
        if op:
            grocery_list.append(
                ["Parle's Wafers", GroceryPrice(clicked_grocery10.get()), GroceryQty(clicked_grocery10.get()),
                 Spaces(40 - len("Parle's Wafers"))])
            messagebox.showinfo("Product Status",
                                "Parle's Cream & Onion Wafers (" + clicked_grocery10.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Parle's Cream & Onion Wafers (" + clicked_grocery10.get() + ") is not added to the cart.")

    add_grocery1 = Button(lf_grocery1, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG1).place(x=60, y=245)
    add_grocery2 = Button(lf_grocery2, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG2).place(x=60, y=245)
    add_grocery3 = Button(lf_grocery3, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG3).place(x=60, y=245)
    add_grocery4 = Button(lf_grocery4, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG4).place(x=60, y=245)
    add_grocery5 = Button(lf_grocery5, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG5).place(x=60, y=245)
    add_grocery6 = Button(lf_grocery6, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG6).place(x=60, y=245)
    add_grocery7 = Button(lf_grocery7, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG7).place(x=60, y=245)
    add_grocery8 = Button(lf_grocery8, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG8).place(x=60, y=245)
    add_grocery9 = Button(lf_grocery9, text="Add Item", bg="green", fg="white", font="times 9 bold",
                          command=AddG9).place(x=60, y=245)
    add_grocery10 = Button(lf_grocery10, text="Add Item", bg="green", fg="white", font="times 9 bold",
                           command=AddG10).place(x=60, y=245)


def ElectronicsCall():
    HideAllFrames()
    Electronics_Label = Label(Products_frame, text="Electronics", font="times 15 bold", fg="gold", bg="black").grid(
    ...
    label_electronics_7 = Label(lf_electronics7, image=electronics7_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                        padx=8)
    label_electronics_8 = Label(lf_electronics8, image=electronics8_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                        padx=7)
    label_electronics_9 = Label(lf_electronics9, image=electronics9_image, bd=2, justify="center").grid(row=0, column=0)
    label_electronics_10 = Label(lf_electronics10, image=electronics10_image, bd=2, justify="center").grid(row=0,
                                                                                                           column=0,
                                                                                                           padx=7)
    name_electronics1 = Label(lf_electronics1, text="Redmi Note 9 Pro(Storage:64GB)", font="arial 9",
                              justify="center").grid(row=1, column=0, padx=3)
    name_electronics2 = Label(lf_electronics2, text="OnePlus 8T 5G(Storage:128GB)", font="arial 9",
                              justify="center").grid(row=1, column=0, padx=6)
    name_electronics3 = Label(lf_electronics3, text="boAt Bassheads Wired Earphones", font="arial 9",
                              justify="center").grid(row=1, column=0)
    name_electronics4 = Label(lf_electronics4, text="JBL T460BT On-Ear Headphones", font="arial 9",
                              justify="center").grid(row=1, column=0, padx=3)
    name_electronics5 = Label(lf_electronics5, text="Logitech M221 Wireless Mouse", font="arial 9",
                              justify="center").grid(row=1, column=0, padx=9)
    name_electronics6 = Label(lf_electronics6, text="HP Pavilion 15.6-inch", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_electronics6 = Label(lf_electronics6, text="FHD Gaming Laptop", font="arial 9", justify="center").grid(row=2,
                                                                                                                column=0)
    name_electronics6 = Label(lf_electronics6, text="Storage:1TB HDD + 256GB SSD", font="arial 9",
                              justify="center").grid(row=3, column=0, padx=6)
    price_electronics6 = Label(lf_electronics6, text="Price: Rs.67,900", font="arial 9 bold", justify="center").grid(
        row=4, column=0)
    name_electronics7 = Label(lf_electronics7, text="Acer Aspire 5 15.6 inch Laptop", font="arial 9",
                              justify="center").grid(row=1, column=0)
    name_electronics7 = Label(lf_electronics7, text="Intel Core i5 11th Generation", font="arial 9",
                              justify="center").grid(row=2, column=0)
    name_electronics7 = Label(lf_electronics7, text="Storage: 512GB SSD", font="arial 9", justify="center").grid(row=3,
                                                                                                                 column=0)
    price_electronics7 = Label(lf_electronics7, text="Price: Rs.49,990", font="arial 9 bold", justify="center").grid(
        row=4, column=0)
    name_electronics8 = Label(lf_electronics8, text="OnePlus Y Series Full HD", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_electronics8 = Label(lf_electronics8, text="LED Smart Android TV 43Y1", font="arial 9", justify="center").grid(
        row=2, column=0)
    price_electronics8 = Label(lf_electronics8, text="Price: Rs.15,499  Inches: 32", font="arial 9 bold",
                               justify="center").grid(row=3, column=0)
    name_electronics9 = Label(lf_electronics9, text="Noise Colorfit Pro 2 Smartwatch", font="arial 9",
                              justify="center").grid(row=1, column=0, padx=6)
    name_electronics10 = Label(lf_electronics10, text="EPSON L3115 Color A4", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_electronics10 = Label(lf_electronics10, text="All in ONE Printer", font="arial 9", justify="center").grid(
        row=2, column=0)
    price_electronics10 = Label(lf_electronics10, text="Price: Rs.12,250", font="arial 9 bold", justify="center").grid(
        row=3, column=0)
    label_clr_electronics1 = Label(lf_electronics1, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics2 = Label(lf_electronics2, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics3 = Label(lf_electronics3, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics4 = Label(lf_electronics4, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics5 = Label(lf_electronics5, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics6 = Label(lf_electronics6, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics7 = Label(lf_electronics7, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics8 = Label(lf_electronics8, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics9 = Label(lf_electronics9, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                y=218)
    label_clr_electronics10 = Label(lf_electronics10, text="Colour:", bd=1, font="arial 9", justify="left").place(x=5,
                                                                                                                  y=218)
    ...
    def AddE1():
        ...
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(
                ["Redmi Note 9 Pro", 13650, "13,650", clicked_electronics1.get(), Spaces(40 - len("Redmi Note 9 Pro"))])
            messagebox.showinfo("Product Status",
                                "Redmi Note 9 Pro(Storage:64GB) (" + clicked_electronics1.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Redmi Note 9 Pro(Storage:64GB) (" + clicked_electronics1.get() + ") is not added to the cart.")
    ...
    def AddE2():
        ...
        if op:
            electronics_list.append(
                ["OnePlus 8T 5G", 42999, "42,999", clicked_electronics2.get(), Spaces(40 - len("OnePlus 8T 5G"))])
            messagebox.showinfo("Product Status",
                                "OnePlus 8T 5G(Storage:128GB) (" + clicked_electronics2.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "OnePlus 8T 5G(Storage:128GB) (" + clicked_electronics2.get() + ") is not added to the cart.")
    ...
    def AddE3():
        ...
        if op:
            electronics_list.append(
                ["boAt Earphones", 499, "499", clicked_electronics3.get(), Spaces(40 - len("boAt Earphones"))])
            messagebox.showinfo("Product Status",
                                "boAt Bassheads Wired Earphones (" + clicked_electronics3.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "boAt Bassheads Wired Earphones (" + clicked_electronics3.get() + ") is not added to the cart.")
    ...
    def AddE4():
        ...
        if op:
            electronics_list.append(
                ["JBL Headphones", 2799, "2,799", clicked_electronics4.get(), Spaces(40 - len("JBL Headphones"))])
            messagebox.showinfo("Product Status",
                                "JBL T460BT On-Ear Headphones (" + clicked_electronics4.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "JBL T460BT On-Ear Headphones (" + clicked_electronics4.get() + ") is not added to the cart.")
    ...
    def AddE5():
        ...
        if op:
            electronics_list.append(
                ["Logitech Mouse", 699, "699", clicked_electronics5.get(), Spaces(40 - len("Logitech Mouse"))])
            messagebox.showinfo("Product Status",
                                "Logitech M221 Wireless Mouse (" + clicked_electronics5.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Logitech M221 Wireless Mouse (" + clicked_electronics5.get() + ") is not added to the cart.")

    def AddE6():
        global electronics_list
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(["HP Pavilion Laptop", 67900, "67,900", clicked_electronics6.get(),
                                     Spaces(40 - len("HP Pavilion Laptop"))])
            messagebox.showinfo("Product Status",
                                "HP Pavilion 15.6-inch FHD Gaming Laptop Storage:1TB HDD + 256GB SSD (" + clicked_electronics6.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "HP Pavilion 15.6-inch FHD Gaming Laptop Storage:1TB HDD + 256GB SSD (" + clicked_electronics6.get() + ") is not added to the cart.")

    def AddE7():
        global electronics_list
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(["Acer Aspire 5 Laptop", 49990, "49,990", clicked_electronics7.get(),
                                     Spaces(40 - len("Acer Aspire 5 Laptop"))])
            messagebox.showinfo("Product Status",
                                "Acer Aspire 5 15.6 inch Laptop Intel Core i5 11th Generation Storage: 512GB SSD (" + clicked_electronics7.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Acer Aspire 5 15.6 inch Laptop Intel Core i5 11th Generation Storage: 512GB SSD (" + clicked_electronics7.get() + ") is not added to the cart.")

    def AddE8():
        global electronics_list
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(["OnePlus Android TV", 15499, "15,499", clicked_electronics8.get(),
                                     Spaces(40 - len("OnePlus Android TV"))])
            messagebox.showinfo("Product Status",
                                "OnePlus Y Series Full HD LED Smart Android TV 43Y1 (" + clicked_electronics8.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "OnePlus Y Series Full HD LED Smart Android TV 43Y1 (" + clicked_electronics8.get() + ") is not added to the cart.")

    def AddE9():
        global electronics_list
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(["Noise Colorfit Smartwatch", 2999, "2,999", clicked_electronics9.get(),
                                     Spaces(40 - len("Noise Colorfit Smartwatch"))])
            messagebox.showinfo("Product Status",
                                "Noise Colorfit Pro 2 Smartwatch (" + clicked_electronics9.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Noise Colorfit Pro 2 Smartwatch (" + clicked_electronics9.get() + ") is not added to the cart.")

    def AddE10():
        global electronics_list
        op = messagebox.askyesno("Purchase Confirmation", "Are you sure that you want to add this item to the cart?")
        if op:
            electronics_list.append(["EPSON Color Printer", 12250, "12,250", clicked_electronics10.get(),
                                     Spaces(40 - len("EPSON Color Printer"))])
            messagebox.showinfo("Product Status",
                                "EPSON L3115 Color A4 All in ONE Printer (" + clicked_electronics10.get() + ") is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "EPSON L3115 Color A4 All in ONE Printer (" + clicked_electronics10.get() + ") is not added to the cart.")
    ...
    add_electronics1 = Button(lf_electronics1, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE1).place(x=120, y=245)
    add_electronics2 = Button(lf_electronics2, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE2).place(x=120, y=245)
    add_electronics3 = Button(lf_electronics3, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE3).place(x=120, y=245)
    add_electronics4 = Button(lf_electronics4, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE4).place(x=120, y=245)
    add_electronics5 = Button(lf_electronics5, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE5).place(x=120, y=245)
    add_electronics6 = Button(lf_electronics6, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE6).place(x=60, y=245)
    add_electronics7 = Button(lf_electronics7, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE7).place(x=60, y=245)
    add_electronics8 = Button(lf_electronics8, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE8).place(x=60, y=245)
    add_electronics9 = Button(lf_electronics9, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddE9).place(x=120, y=245)
    add_electronics10 = Button(lf_electronics10, text="Add Item", bg="green", fg="white", font="times 9 bold",
                               command=AddE10).place(x=60, y=245)


def SportsGymCall():
    HideAllFrames()
    Sports_Gym_Label = Label(Products_frame, text="Sports and Gym Equipment", font="times 15 bold", fg="gold",
                             bg="black").grid(row=0, column=0, padx=10)
    ...
    label_sportsgym_2 = Label(lf_sportsgym2, image=sportsgym2_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=7)
    label_sportsgym_3 = Label(lf_sportsgym3, image=sportsgym3_image, bd=2, justify="center").grid(row=0, column=0)
    label_sportsgym_4 = Label(lf_sportsgym4, image=sportsgym4_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_sportsgym_5 = Label(lf_sportsgym5, image=sportsgym5_image, bd=2, justify="center").grid(row=0, column=0)
    label_sportsgym_6 = Label(lf_sportsgym6, image=sportsgym6_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_sportsgym_7 = Label(lf_sportsgym7, image=sportsgym7_image, bd=2, justify="center").grid(row=0, column=0)
    label_sportsgym_8 = Label(lf_sportsgym8, image=sportsgym8_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    ...
    name_sportsgym1 = Label(lf_sportsgym1, text="GM Purist KW 202 Cricket Bat", font="arial 9", justify="center").grid(
        row=1, column=0, padx=13)
    name_sportsgym2 = Label(lf_sportsgym2, text="GTS Dezir Cricket Leather Ball", font="arial 9",
                            justify="center").grid(row=1, column=0)
    name_sportsgym3 = Label(lf_sportsgym3, text="Yonex GR 303 Badminton Racquet", font="arial 9",
                            justify="center").grid(row=1, column=0)
    name_sportsgym4 = Label(lf_sportsgym4, text="Strauss Maxis Pro Shuttlecock", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_sportsgym5 = Label(lf_sportsgym5, text="Stag 4 Star Table Tennis Kit", font="arial 9", justify="center").grid(
        row=1, column=0, padx=14)
    name_sportsgym6 = Label(lf_sportsgym6, text="Cockatoo CTM14A 2.5HP", font="arial 9", justify="center").grid(row=1,
                                                                                                                column=0)
    name_sportsgym6 = Label(lf_sportsgym6, text="Treadmill with Auto-Incline", font="arial 9", justify="center").grid(
        row=2, column=0)
    name_sportsgym7 = Label(lf_sportsgym7, text="Aurion HEX Rubber Coated Cast", font="arial 9", justify="center").grid(
        row=1, column=0, padx=4)
    name_sportsgym7 = Label(lf_sportsgym7, text="Iron Hexagonal Dumbbells for", font="arial 9", justify="center").grid(
        row=2, column=0)
    name_sportsgym7 = Label(lf_sportsgym7, text="Professional Exercise", font="arial 9", justify="center").grid(row=3,
                                                                                                                column=0)
    name_sportsgym8 = Label(lf_sportsgym8, text="SPORTAL Foldable Multi Bench", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_sportsgym8 = Label(lf_sportsgym8, text="Press Workout Machine", font="arial 9", justify="center").grid(row=2,
                                                                                                                column=0)
    name_sportsgym9 = Label(lf_sportsgym9, text="Reach Air Bike Exercise Cycle", font="arial 9", justify="center").grid(
        row=1, column=0, padx=12)
    name_sportsgym10 = Label(lf_sportsgym10, text="RMOUR Filled Heavy Punch Bag", font="arial 9",
                             justify="center").grid(row=1, column=0, padx=5)
    ...
    def AddS6():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Cockatoo CTM14A 2.5HP Treadmill with Auto-Incline is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Cockatoo CTM14A 2.5HP Treadmill with Auto-Incline is not added to the cart.")
    ...
    def AddS7():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Aurion HEX Rubber Coated Cast Iron Hexagonal Dumbbells for Professional Exercise is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Aurion HEX Rubber Coated Cast Iron Hexagonal Dumbbells for Professional Exercise is not added to the cart.")
    ...
    def AddS8():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "SPORTAL Foldable Multi Bench Press Workout Machine is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "SPORTAL Foldable Multi Bench Press Workout Machine is not added to the cart.")
    ...
    add_sportsgym1 = Button(lf_sportsgym1, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS1).place(x=68, y=245)
    add_sportsgym2 = Button(lf_sportsgym2, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS2).place(x=68, y=245)
    add_sportsgym3 = Button(lf_sportsgym3, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS3).place(x=68, y=245)
    add_sportsgym4 = Button(lf_sportsgym4, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS4).place(x=68, y=245)
    add_sportsgym5 = Button(lf_sportsgym5, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS5).place(x=68, y=245)
    add_sportsgym6 = Button(lf_sportsgym6, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS6).place(x=68, y=245)
    add_sportsgym7 = Button(lf_sportsgym7, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS7).place(x=68, y=245)
    add_sportsgym8 = Button(lf_sportsgym8, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS8).place(x=68, y=245)
    add_sportsgym9 = Button(lf_sportsgym9, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddS9).place(x=68, y=245)
    add_sportsgym10 = Button(lf_sportsgym10, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddS10).place(x=68, y=245)


def FurnitureCall():
    HideAllFrames()
    Furniture_Label = Label(Products_frame, text="Furniture", font="times 15 bold", fg="gold", bg="black").grid(row=0,
                                                                                                                column=0,
                                                                                                                padx=20)
    ...
    label_furniture_2 = Label(lf_furniture2, image=furniture2_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_furniture_3 = Label(lf_furniture3, image=furniture3_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_furniture_4 = Label(lf_furniture4, image=furniture4_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_furniture_5 = Label(lf_furniture5, image=furniture5_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    ...
    label_furniture_8 = Label(lf_furniture8, image=furniture8_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_furniture_9 = Label(lf_furniture9, image=furniture9_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                  padx=8)
    label_furniture_10 = Label(lf_furniture10, image=furniture10_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                     padx=8)
    name_furniture1 = Label(lf_furniture1, text="Julian Wood 2 Door Wardrobe", font="arial 9", justify="center").grid(
        row=1, column=0, padx=10)
    name_furniture2 = Label(lf_furniture2, text="Zuari Niko Engineered Wood", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture2 = Label(lf_furniture2, text="3 Door Wardrobe", font="arial 9", justify="center").grid(row=2,
                                                                                                          column=0)
    name_furniture3 = Label(lf_furniture3, text="TADesign Vinicio Engineered", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture3 = Label(lf_furniture3, text="Wood 3 Door Wardrobe", font="arial 9", justify="center").grid(row=2,
                                                                                                               column=0)
    name_furniture4 = Label(lf_furniture4, text="Flipkart Perfect Homes Opus", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture4 = Label(lf_furniture4, text="Engineered Wood", font="arial 9", justify="center").grid(row=2,
                                                                                                          column=0)
    name_furniture4 = Label(lf_furniture4, text="Queen Box Bed", font="arial 9", justify="center").grid(row=3, column=0)
    name_furniture5 = Label(lf_furniture5, text="Forzza Jasper Engineered", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture5 = Label(lf_furniture5, text="Wood King Box Bed", font="arial 9", justify="center").grid(row=2,
                                                                                                            column=0)
    name_furniture6 = Label(lf_furniture6, text="Zuari Wood TV Entertainment Unit", font="arial 9",
                            justify="center").grid(row=1, column=0, padx=2)
    name_furniture7 = Label(lf_furniture7, text="Forzza Belfast Engineered Wood", font="arial 9",
                            justify="center").grid(row=1, column=0, padx=5)
    name_furniture7 = Label(lf_furniture7, text="TV Entertainment Unit", font="arial 9", justify="center").grid(row=2,
                                                                                                                column=0)
    name_furniture8 = Label(lf_furniture8, text="Kurlon Crescent Leatherette", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture8 = Label(lf_furniture8, text="3 + 1 + 1 Black Sofa Set", font="arial 9", justify="center").grid(
        row=2, column=0)
    name_furniture9 = Label(lf_furniture9, text="Suncrown Furniture Solid Wood", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_furniture9 = Label(lf_furniture9, text="Fabric 6 Seater Sofa", font="arial 9", justify="center").grid(row=2,
                                                                                                               column=0)
    name_furniture9 = Label(lf_furniture9, text="3 + 2 + 1 Vanilla Cream Sofa Set", font="arial 9",
                            justify="center").grid(row=3, column=0)
    name_furniture10 = Label(lf_furniture10, text="Allie Wood Solid Wood", font="arial 9", justify="center").grid(row=1,
                                                                                                                  column=0)
    name_furniture10 = Label(lf_furniture10, text="6 Seater Dining Set", font="arial 9", justify="center").grid(row=2,
                                                                                                                column=0)
    ...
    def AddF2():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Zuari Niko Engineered Wood 3 Door Wardrobe is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Zuari Niko Engineered Wood 3 Door Wardrobe is not added to the cart.")
    ...
    def AddF3():
        ...
        if op:
            furniture_list.append(
                ["TADesign 3 Door Wardrobe", 34999, "34,999", Spaces(40 - len("TADesign 3 Door Wardrobe"))])
            messagebox.showinfo("Product Status",
                                "TADesign Vinicio Engineered Wood 3 Door Wardrobe is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "TADesign Vinicio Engineered Wood 3 Door Wardrobe is not added to the cart.")
    ...
    def AddF4():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Flipkart Perfect Homes Opus Engineered Wood Queen Box Bed is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Flipkart Perfect Homes Opus Engineered Wood Queen Box Bed is not added to the cart.")
    ...
    def AddF5():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Forzza Jasper Engineered Wood King Box Bed is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Forzza Jasper Engineered Wood King Box Bed is not added to the cart.")
    ...
    def AddF7():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Forzza Belfast Engineered Wood TV Entertainment Unit is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Forzza Belfast Engineered Wood TV Entertainment Unit is not added to the cart.")
    ...
    def AddF8():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Kurlon Crescent Leatherette 3 + 1 + 1 Black Sofa Set is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Kurlon Crescent Leatherette 3 + 1 + 1 Black Sofa Set is not added to the cart.")
    ...
    def AddF9():
        ...
        if op:
            furniture_list.append(
                ["Suncrown Cream Sofa Set", 36854, "36,854", Spaces(40 - len("Suncrown Cream Sofa Set"))])
            messagebox.showinfo("Product Status",
                                "Suncrown Furniture Solid Wood Fabric 6 Seater Sofa 3 + 2 + 1 Vanilla Cream Sofa Set is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Suncrown Furniture Solid Wood Fabric 6 Seater Sofa 3 + 2 + 1 Vanilla Cream Sofa Set is not added to the cart.")
    ...
    def AddF10():
        ...
        if op:
            furniture_list.append(
                ["Allie 6 Seater Dining Set", 26999, "26,999", Spaces(40 - len("Allie 6 Seater Dining Set"))])
            messagebox.showinfo("Product Status",
                                "Allie Wood Solid Wood 6 Seater Dining Set is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status", "Allie Wood Solid Wood 6 Seater Dining Set is not added to the cart.")

    add_furniture1 = Button(lf_furniture1, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF1).place(x=68, y=245)
    add_furniture2 = Button(lf_furniture2, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF2).place(x=68, y=245)
    add_furniture3 = Button(lf_furniture3, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF3).place(x=68, y=245)
    add_furniture4 = Button(lf_furniture4, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF4).place(x=68, y=245)
    add_furniture5 = Button(lf_furniture5, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF5).place(x=68, y=245)
    add_furniture6 = Button(lf_furniture6, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF6).place(x=68, y=245)
    add_furniture7 = Button(lf_furniture7, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF7).place(x=68, y=245)
    add_furniture8 = Button(lf_furniture8, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF8).place(x=68, y=245)
    add_furniture9 = Button(lf_furniture9, text="Add Item", bg="green", fg="white", font="times 9 bold",
                            command=AddF9).place(x=68, y=245)
    add_furniture10 = Button(lf_furniture10, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddF10).place(x=68, y=245)


def AppliancesCall():
    HideAllFrames()
    Appliances_Label = Label(Products_frame, text="Appliances", font="times 15 bold", fg="gold", bg="black").grid(row=0,
                                                                                                                  column=0,
                                                                                                                  padx=20)
    ...
    label_appliances_3 = Label(lf_appliances3, image=appliances3_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                     padx=7)
    label_appliances_4 = Label(lf_appliances4, image=appliances4_image, bd=2, justify="center").grid(row=0, column=0)
    label_appliances_5 = Label(lf_appliances5, image=appliances5_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                     padx=7)
    label_appliances_6 = Label(lf_appliances6, image=appliances6_image, bd=2, justify="center").grid(row=0, column=0)
    label_appliances_7 = Label(lf_appliances7, image=appliances7_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                     padx=8)
    label_appliances_8 = Label(lf_appliances8, image=appliances8_image, bd=2, justify="center").grid(row=0, column=0)
    label_appliances_9 = Label(lf_appliances9, image=appliances9_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                     padx=22)
    label_appliances_10 = Label(lf_appliances10, image=appliances10_image, bd=2, justify="center").grid(row=0, column=0,
                                                                                                        padx=6)
    name_appliances1 = Label(lf_appliances1, text="Whirlpool 7kg Automatic Top Load", font="arial 9",
                             justify="center").grid(row=1, column=0)
    name_appliances2 = Label(lf_appliances2, text="IFB 6kg Fully Automatic Front Load", font="arial 9",
                             justify="center").grid(row=1, column=0)
    name_appliances3 = Label(lf_appliances3, text="Samsung 1.5 Ton 5 Star", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_appliances3 = Label(lf_appliances3, text="Split Inverter AC", font="arial 9", justify="center").grid(row=2,
                                                                                                              column=0)
    name_appliances4 = Label(lf_appliances4, text="LG 260L Double Door Refrigerator", font="arial 9",
                             justify="center").grid(row=1, column=0)
    name_appliances5 = Label(lf_appliances5, text="IFB 20 L Convection", font="arial 9", justify="center").grid(row=1,
                                                                                                                column=0)
    name_appliances5 = Label(lf_appliances5, text="Microwave Oven 20SC2", font="arial 9", justify="center").grid(row=2,
                                                                                                                 column=0)
    name_appliances6 = Label(lf_appliances6, text="Bajaj GX1 500W Mixer Grinder", font="arial 9",
                             justify="center").grid(row=1, column=0, padx=12)
    name_appliances7 = Label(lf_appliances7, text="Balzano OX218 Electric Kettle", font="arial 9",
                             justify="center").grid(row=1, column=0)
    name_appliances8 = Label(lf_appliances8, text="Elica WDFL 606 HAC MS NERO", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_appliances8 = Label(lf_appliances8, text="Auto Clean Wall Mounted Chimney", font="arial 9",
                             justify="center").grid(row=2, column=0)
    name_appliances9 = Label(lf_appliances9, text="Kent Ace 8 L Water Purifier", font="arial 9", justify="center").grid(
        row=1, column=0)
    name_appliances10 = Label(lf_appliances10, text="Eureka Forbes Quick Clean DX", font="arial 9",
                              justify="center").grid(row=1, column=0)
    name_appliances10 = Label(lf_appliances10, text="Dry Vacuum Cleaner", font="arial 9", justify="center").grid(row=2,
                                                                                                                 column=0)
    ...
    def AddA2():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "IFB 6kg Fully Automatic Front Load is successfully added to the cart.")
        else:
    ...
    def AddA3():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "Samsung 1.5 Ton 5 Star Split Inverter AC is successfully added to the cart.")
        else:
    ...
    def AddA5():
        ...
        if op:
            ...
            messagebox.showinfo("Product Status",
                                "IFB 20 L Convection Microwave Oven 20SC2 is successfully added to the cart.")
        else:
    ...
    def AddA8():
        ...
        if op:
            appliances_list.append(
                ["Elica Wall Mounted Chimney", 11999, "11,999", Spaces(40 - len("Elica Wall Mounted Chimney"))])
            messagebox.showinfo("Product Status",
                                "Elica WDFL 606 HAC MS NERO Auto Clean Wall Mounted Chimney is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Elica WDFL 606 HAC MS NERO Auto Clean Wall Mounted Chimney is not added to the cart.")
    ...
    def AddA9():
        ...
        if op:
            appliances_list.append(
                ["Kent Ace Water Purifier", 12499, "12,499", Spaces(40 - len("Kent Ace Water Purifier"))])
    ...
    def AddA10():
        ...
        if op:
            appliances_list.append(
                ["Eureka Dry Vacuum Cleaner", 3249, "3,249", Spaces(40 - len("Eureka Dry Vacuum Cleaner"))])
            messagebox.showinfo("Product Status",
                                "Eureka Forbes Quick Clean DX Dry Vacuum Cleaner is successfully added to the cart.")
        else:
            messagebox.showinfo("Product Status",
                                "Eureka Forbes Quick Clean DX Dry Vacuum Cleaner is not added to the cart.")

    add_appliances1 = Button(lf_appliances1, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA1).place(x=68, y=245)
    add_appliances2 = Button(lf_appliances2, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA2).place(x=68, y=245)
    add_appliances3 = Button(lf_appliances3, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA3).place(x=68, y=245)
    add_appliances4 = Button(lf_appliances4, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA4).place(x=68, y=245)
    add_appliances5 = Button(lf_appliances5, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA5).place(x=68, y=245)
    add_appliances6 = Button(lf_appliances6, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA6).place(x=68, y=245)
    add_appliances7 = Button(lf_appliances7, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA7).place(x=68, y=245)
    add_appliances8 = Button(lf_appliances8, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA8).place(x=68, y=245)
    add_appliances9 = Button(lf_appliances9, text="Add Item", bg="green", fg="white", font="times 9 bold",
                             command=AddA9).place(x=68, y=245)
    add_appliances10 = Button(lf_appliances10, text="Add Item", bg="green", fg="white", font="times 9 bold",
                              command=AddA10).place(x=68, y=245)


Grocery_button = Button(Button_frame, text="Grocery", font="times 20 bold", width=17, bd=6, bg="cadetblue", fg="white",
                        activebackground="light blue", command=GroceryCall)
Grocery_button.grid(row=0, column=0, padx=5, pady=5)
Electronics_button = Button(Button_frame, text="Electronics", font="times 20 bold", width=17, bd=6, bg="cadetblue",
                            fg="white", activebackground="light blue", command=ElectronicsCall)
Electronics_button.grid(row=1, column=0, padx=5, pady=5)
Sports_Gym_button = Button(Button_frame, text="Sports and Gym", font="times 20 bold", width=17, bd=6, bg="cadetblue",
                           fg="white", activebackground="light blue", command=SportsGymCall)
Sports_Gym_button.grid(row=2, column=0, padx=5, pady=5)
Furniture_button = Button(Button_frame, text="Furniture", font="times 20 bold", width=17, bd=6, bg="cadetblue",
                          fg="white", activebackground="light blue", command=FurnitureCall)
Furniture_button.grid(row=3, column=0, padx=5, pady=5)
Appliances_button = Button(Button_frame, text="Appliances", font="times 20 bold", width=17, bd=6, bg="cadetblue",
                           fg="white", activebackground="light blue", command=AppliancesCall)
Appliances_button.grid(row=4, column=0, padx=5, pady=5)
Coupon_frame = LabelFrame(page, bd=2, relief="groove", text="MEGA SALE!!!", fg="green", font="arial 16 bold")
Coupon_frame.place(x=2, y=450, width=300, height=230)
Coupon_1 = Label(Coupon_frame, text="Get 15% Off on your purchase(upto Rs.500)", font="times 12", fg="yellow",
                 bg="brown")
Coupon_2 = Label(Coupon_frame, text="Get 10% Off on your purchase(upto Rs.750)", font="times 12", fg="yellow",
                 bg="brown")
Coupon_3 = Label(Coupon_frame, text="Get 5% Off on your purchase(upto Rs.1000)", font="times 12", fg="yellow",
                 bg="brown")
...
def Bill():
    op = messagebox.askyesno("Bill Generation Confirmation",
                             "Products cannot be added or removed during bill generation. Are you sure that you have finished shopping?")
    if op:
        ...
        if max_discount == discount[0]:
            suggest = Label(page, bd=1, text="Suggested : 15% Off upto Rs.500", font="times 12", fg="blue").place(x=545,
                                                                                                                  y=480)
        elif max_discount == discount[1]:
            suggest = Label(page, bd=1, text="Suggested : 10% Off upto Rs.750", font="times 12", fg="blue").place(x=545,
                                                                                                                  y=480)
        else:
            suggest = Label(page, bd=1, text="Suggested : 5% Off upto Rs.1000", font="times 12", fg="blue").place(x=545,
                                                                                                                  y=480)

        def GenBill(d, choice):
            bill_area = LabelFrame(page, bd=2, relief="groove")
            ...
            bill_txt_area.insert(END, Spaces(40) + "The Cartmax\n" + Spaces(90, '*') + "\n")
            if len(grocery_list) > 0:
                bill_txt_area.insert(END, "Grocery Product(s)\n\nProduct Name" + Spaces(28) + "Price" + Spaces(
                    25) + "Quantity\n")
                for i in grocery_list:
                    bill_txt_area.insert(END,
                                         i[0] + i[3] + "Rs." + str(i[1]) + Spaces(27 - len(str(i[1]))) + i[2] + "\n")
                bill_txt_area.insert(END,
            ...
            if len(sportsgym_list) > 0:
                ...
                bill_txt_area.insert(END, "\nTotal Sports and Gym Equipment Price : Rs." + str(
                    sportsgym_price) + "\n" + Spaces(90, '*') + "\n")
            ...
            bill_txt_area.insert(END, "\nTotal Price(after discount) = Rs." + str(total_price))
            save_button = Button(page, text="Save Invoice", font="times 20 bold", bd=6, bg="skyblue", width=10,
                                 fg="white", command=lambda: save_invoice(bill_txt_area.get("1.0", END)))
            save_button.place(x=1120, y=600)

        Coupon_frame_2 = LabelFrame(page, bd=2, relief="groove", text="Apply a Coupon", fg="green",
                                    font="arial 16 bold").place(x=500, y=150, width=380, height=300)
        Coupon_apply1 = Button(page, text="15% Off upto Rs.500", font="times 20 bold", width=17, bd=6,
                               bg="cadetblue", fg="white", activebackground="light blue",
                               command=lambda: GenBill(discount[0], 1))
        Coupon_apply1.place(x=540, y=190)
        Coupon_apply2 = Button(page, text="10% Off upto Rs.750", font="times 20 bold", width=17, bd=6,
                               bg="cadetblue", fg="white", activebackground="light blue",
                               command=lambda: GenBill(discount[1], 2))
        Coupon_apply2.place(x=540, y=280)
        Coupon_apply3 = Button(page, text="5% Off upto Rs.1000", font="times 20 bold", width=17, bd=6,
                               bg="cadetblue", fg="white", activebackground="light blue",
                               command=lambda: GenBill(discount[2], 3))
        Coupon_apply3.place(x=540, y=370)
    else:
        messagebox.showinfo("Bill Generation Confirmation", "You can continue shopping now.")


login_button = Button(Heading, bd=3, text="Logout", font="times 17 bold", bg="skyblue", fg="white", command=logout)
bill_gen_button = Button(Heading, bd=3, text="Checkout", font="times 17 bold", bg="skyblue", fg="white",
                         activebackground="purple", command=Bill)
